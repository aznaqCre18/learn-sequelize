                ...
                if (_.startsWith(_this.sql, "BEGIN TRANSACTION")) {
                  ...
                  connection.prepare(newSql, function (err, stmt) {
                    ...
                    stmt.execute(params, function (err2, result, outparams) {
                      debug("executed(".concat(_this.connection.uuid || "default", "):").concat(newSql, " ").concat(parameters ? JSON.stringify(parameters) : ""));

                      if (benchmark) {
                        _this.sequelize.log("Executed (".concat(_this.connection.uuid || "default", "): ").concat(newSql, " ").concat(parameters ? JSON.stringify(parameters) : ""), Date.now() - queryBegin, _this.options);
                      }
