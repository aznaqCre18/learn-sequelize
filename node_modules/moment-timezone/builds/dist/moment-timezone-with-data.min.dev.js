...
!function (c, M) {
  ...
  function r(c) {
    var M = c.split("|"),
        ...
        o = M[3].split(""),
        z = M[4].split(" ");
    return L(A), L(o), L(z), function (c, M) {
      for (var A = 0; A < M; A++) {
  ...
  function l() {
    ...
    var A,
      ...
      for (A = 1; A < 48; A++) {
        (M = new W(new Date(o, A, 1))).offset !== z.offset && (c = X(z, M), b.push(c), b.push(new W(new Date(c.at + 6e4)))), z = M;
      }

      for (A = 0; A < 4; A++) {
        b.push(new W(new Date(o + A, 0, 1))), b.push(new W(new Date(o + A, 6, 1)));
      }
  ...
  function t(c) {
    ...
    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) {
      z = T(A = (o = c[M].split("|"))[0]), a[z] = c[M], i[z] = A, B(z, o[2].split(" "));
    }
  }
  ...
  function m(c) {
    ...
    for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) {
      o = T((A = c[M].split("|"))[0]), z = T(A[1]), p[o] = z, i[o] = A[0], p[z] = o, i[z] = A[1];
    }
  }

  function C(c) {
    t(c.zones), m(c.links), function (c) {
      var M, A, o, z;
      if (c && c.length) for (M = 0; M < c.length; M++) {
        A = (z = c[M].split("|"))[0].toUpperCase(), o = z[1].split(" "), n[A] = new d(A, o);
      }
  ...
  function S(c) {
    ...
    return o && !b.isMoment(c) && E(z) && z.add(o.parse(z), "minutes"), z.tz(A), z;
  }

  (A < 2 || 2 == A && o < 6) && R("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + b.version + ". See momentjs.com"), N.prototype = {
    ...
    parse: function parse(c) {
      ...
      for (z = 0; z < n; z++) {
        if (M = a[z], A = a[z + 1], o = a[z ? z - 1 : z], M < A && S.moveAmbiguousForward ? M = A : o < M && S.moveInvalidForward && (M = o), b < p[z] - 6e4 * M) return a[z];
      }
    ...
    offset: function offset(c) {
      return R("zone.offset has been deprecated in favor of zone.utcOffset"), this.offsets[this._index(c)];
    },
  ...
  }, S.version = "0.5.34", S.dataVersion = "", S._zones = a, S._links = p, S._names = i, S._countries = n, S.add = t, S.link = m, S.load = C, S.zone = s, S.zoneExists = function c(M) {
    return c.didShowError || (c.didShowError = !0, R("moment.tz.zoneExists('" + M + "') has been deprecated in favor of !moment.tz.zone('" + M + "')")), !!s(M);
  }, S.guess = function (c) {
    return M && !c || (M = l()), M;
  }, S.names = function () {
    var c,
    ...
    return M.sort();
  }, S.Zone = N, S.unpack = r, S.unpackBase60 = z, S.needsOffset = E, S.moveInvalidForward = !0, S.moveAmbiguousForward = !1, S.countries = function () {
  ...
  b.tz = S, b.defaultZone = null, b.updateOffset = function (c, M) {
    var A,
        o = b.defaultZone;
    if (void 0 === c._z && (o && E(c) && !c._isUTC && (c._d = b.utc(c._a)._d, c.utc().add(o.parse(c), "minutes")), c._z = o), c._z) if (A = c._z.utcOffset(c), Math.abs(A) < 16 && (A /= 60), void 0 !== c.utcOffset) {
      var z = c._z;
    ...
    if (c) {
      ...
      return this._z = s(c), this._z ? b.updateOffset(this, M) : R("Moment Timezone has no data for " + c + ". See http://momentjs.com/timezone/docs/#/data-loading/."), this;
    }

    if (this._z) return this._z.name;
  }, g.zoneName = D(g.zoneName), g.zoneAbbr = D(g.zoneAbbr), g.utc = h(g.utc), g.local = h(g.local), g.utcOffset = (P = g.utcOffset, function () {
    return 0 < arguments.length && (this._z = null), P.apply(this, arguments);
  }), b.tz.setDefault = function (c) {
    return (A < 2 || 2 == A && o < 9) && R("Moment Timezone setDefault() requires Moment.js >= 2.9.0. You are using Moment.js " + b.version + "."), b.defaultZone = c ? s(c) : null, b;
  };
